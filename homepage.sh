...
docker run -d -p 3000:3000 --network npmproxy --restart unless-stopped -v /$HOME/docker/homepage:/app/config -v /var/run/docker.sock:/var/run/docker.sock ghcr.io/benphelps/homepage:latest