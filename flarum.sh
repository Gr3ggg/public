...
apt install -y zip unzip curl wget git php php-cli php-fpm php-common php-mbstring php-gd php-xml php-mysql php-curl php-zip mariadb-server nginx expect git
...
# Liste des paquets Flarum à installer
flarum_packages=(
    "flarum-lang/french"
    "bilgehanars/packman:*"
    ...
    "nearata/flarum-ext-discord-widget:*"
...
# Installe les paquets Flarum
for package in "${flarum_packages[@]}"; do
    sudo -u www-data composer require "$package"
done
